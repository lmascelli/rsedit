...
** 📂 Project Structure
    ...
    - core/ (The Engine):
      ...
      - buffer/ (Buffer Module):
        - mod.rs: Contains the generic struct of a buffer that encapsulate a specific buffer strategy (i.e. the GapBuffer)
              
        - gap_buffer.rs: An $O(1)$ text manipulation structures for inserting new char.

        - buffer_trait.rs: This is an abstraction of the buffer behavior that is the base on which the abstraction of the buffer is made. Implementing this trait for a struct permits to compile the whole editor based on that struct.
      ...
      - ui/ (UI Module):
        ...
        - mod.rs: Contains the LayoutNode, made of splits and leaf (windows) that compose windows viewed on the screen and the structures necessary to use it.
